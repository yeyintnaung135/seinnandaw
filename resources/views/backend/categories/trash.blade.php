                                ...
                                <div class="w-100 d-flex justify-content-between align-items-center">
                                    <h3 class="card-title">Trashed list</h3>
                                       <div class=" d-flex">
                                        ...
                                        <form  action="{{ route('categories.multiple.restore') }}" method="POST" id="restoreform">
                                            @csrf
                                       ...
                                       <button id="multipleDelete" onclick="Delete()" type="button" class=" btn btn-danger btn-sm d-none">
                                        ...
                                        <form id="delete_form" action="{{ route('categories.multiple.forcedelete') }}"method="POST">
                                            @csrf
...
<script>
    ...
    $(document).ready(function() {
        $('#productTrashedTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                "url": "{{ route('category.trash.datatable') }}",
            ...
            columns: [
                ...
                {data: 'created_at'},
                {
                    data: 'action',
                    render: function (data, type) {
                        var restore = `<a href="{{route('category.restore',':id')}}" class="btn btn-info btn-sm mr-1" title="Restore product">
                                                <i class="fas fa-trash-restore"></i>
                                                   Restore
                                                </a>`;
